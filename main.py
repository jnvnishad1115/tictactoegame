#!/usr/bin/env python3
"""
🎮 Tic-Tac-Toe Telegram Bot - FIXED Event Loop Issue
"""
...
class Settings:
    ...
    DB_NAME = os.getenv("DB_NAME", "tictactoe_bot")
...
class Database:
    ...
    async def finish_game(self, game_id: str, winner: Optional[str]):
        await self.games.update_one(
            {"game_id": game_id},
            {"$set": {"state": "finished", "winner": winner, "last_activity": datetime.utcnow()}}
        )
        await self.bot_stats.update_one(
            {"_id": "global"},
    ...
    async def get_leaderboard(self, limit: int = 10) -> List[Dict]:
        cursor = self.users.find().sort("stats.wins", -1).limit(limit)
...
class BotCore:
    ...
    async def _cleanup_expired_games(self):
        ...
        async for game in self.db.games.find({
            ...
            await self.db.finish_game(game["game_id"], None)
            try:
    ...
    async def handle_eval(self, client: Client, message: Message):
        ...
        if not code:
            await message.reply("❌ Provide code to execute!")
            return
        
        try:
            import ast
            tree = ast.parse(code, mode='eval')
            allowed_nodes = (ast.Expression, ast.Add, ast.Sub, ast.Mult, ast.Div, 
                           ast.Num, ast.BinOp, ast.Name, ast.Load)
    ...
    async def _handle_move(self, client: Client, query: CallbackQuery):
        ...
        if winner:
            game["state"] = "finished"
            await self.db.finish_game(game["game_id"], winner)
        ...
        if game["state"] == "playing" and game["current_turn"] == game.get("ai_symbol"):
            asyncio.create_task(self._handle_ai_move(client, query.message, game))
    
    async def _handle_ai_move(self, message: Message, game: Dict):
    ...
    async def _handle_set_theme(self, client: Client, query: CallbackQuery):
        ...
        game["theme"] = theme
        await self.db.update_game(game["game_id"], {"theme": theme})
...
def main():
    ...
    print("🚀 Starting Tic-Tac-Toe Pro Bot...")
    
    # Validate config
    if settings.API_ID == 123456 or settings.API_HASH == "YOUR_API_HASH" or settings.BOT_TOKEN == "YOUR_BOT_TOKEN":
        print("❌ ERROR: Please set your API credentials in .env file!")
        print("   Create .env with: API_ID, API_HASH, BOT_TOKEN, OWNER_ID")
        sys.exit(1)
    
    if settings.OWNER_ID == 123456789:
        print("⚠️  WARNING: Please set your OWNER_ID in .env file!")
    ...
    async def start_handler(_, message): await bot.handle_start(client, message)
    
    @client.on_message(filters.command("newgame"))
    async def newgame_handler(_, message): await bot.handle_newgame(client, message)
    
    @client.on_message(filters.command("ai"))
    async def ai_handler(_, message): await bot.handle_ai(client, message)
    
    @client.on_message(filters.command("profile"))
    async def profile_handler(_, message): await bot.handle_profile(client, message)
    
    @client.on_message(filters.command("leaderboard"))
    async def leaderboard_handler(_, message): await bot.handle_leaderboard(client, message)
    
    @client.on_message(filters.command("help"))
    async def help_handler(_, message): await bot.handle_help(client, message)
    
    @client.on_message(filters.command("resetgame"))
    async def resetgame_handler(_, message): await bot.handle_resetgame(client, message)
    
    @client.on_message(filters.command("settings"))
    async def settings_handler(_, message): await bot.handle_settings(client, message)
    
    @client.on_message(filters.command("enablebot"))
    async def enablebot_handler(_, message): await bot.handle_enablebot(client, message)
    
    @client.on_message(filters.command("disablebot"))
    async def disablebot_handler(_, message): await bot.handle_disablebot(client, message)
    
    # Owner commands
    @client.on_message(filters.command("stats"))
    async def stats_handler(_, message): await bot.handle_owner_stats(client, message)
    
    @client.on_message(filters.command("broadcast"))
    async def broadcast_handler(_, message): await bot.handle_broadcast(client, message)
    
    @client.on_message(filters.command("active_games"))
    async def active_games_handler(_, message): await bot.handle_active_games(client, message)
    
    @client.on_message(filters.command("users"))
    async def users_handler(_, message): await bot.handle_users(client, message)
    
    @client.on_message(filters.command("groups"))
    async def groups_handler(_, message): await bot.handle_groups(client, message)
    
    @client.on_message(filters.command("restart"))
    async def restart_handler(_, message): await bot.handle_restart(client, message)
    
    @client.on_message(filters.command("shutdown"))
    async def shutdown_handler(_, message): await bot.handle_shutdown(client, message)
    
    @client.on_message(filters.command("maintenance"))
    async def maintenance_handler(_, message): await bot.handle_maintenance(client, message)
    
    @client.on_message(filters.command("block"))
    async def block_handler(_, message): await bot.handle_block(client, message)
    
    @client.on_message(filters.command("unblock"))
    async def unblock_handler(_, message): await bot.handle_unblock(client, message)
    
    @client.on_message(filters.command("eval"))
    async def eval_handler(_, message): await bot.handle_eval(client, message)
    
    # Callback queries
    @client.on_callback_query()
    async def callback_handler(_, query): await bot.handle_callback(client, query)
